...
export class CrossCorrelation_NumberOfSales_Temperature
  implements ICorrelationImplementaion, IBuildCorrelation
{
  ...
  async run() {
    ...
    this.tableModel.next(await this.buildCorrelationTable());
    this.graphModel.next(await this.buildCorrelationGraph());
  }

  async buildCorrelationTable(): Promise<TableModel> {
  ...
  async buildCorrelationGraph(): Promise<GraphModel> {
    let values: any[] =
...
export class CrossCorrelation_SeatTime_Temperature
  implements ICorrelationImplementaion, IBuildCorrelation
{
  title: string = 'Seat time vs Temperature';
  correlationReturn: CorrelationReturn | undefined;
  tableModel = new Subject<TableModel>();
  graphModel = new Subject<GraphModel>();
  ...
  async run() {
    ...
    this.correlationReturn =
      await this.correlationService.Correlation_SeatTime_Vs_Temperature(
        salesIds,
        this.dialogCrossCorrelationSettings.startDate!,
        this.dialogCrossCorrelationSettings.endDate,
        this.dialogCrossCorrelationSettings.timeResolution!,
