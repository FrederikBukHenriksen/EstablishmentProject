...
namespace EstablishmentProject.test.Repositories
{
    public class RepositoryTest : IntegrationTest
    {
        ...
        public void GetFromIds_Sucess()
        {
            ...
            // Act
            var fetchedEstablishment = unitOfWork.establishmentRepository.GetFromIds(new List<Guid> { insertedEstablishment.Id });

            // Assert
            Assert.NotNull(fetchedEstablishment);
            Assert.Equal(insertedEstablishment.Id, fetchedEstablishment.First().Id);
            Assert.Equal(insertedEstablishment.Name, fetchedEstablishment.First().Name);
        }
