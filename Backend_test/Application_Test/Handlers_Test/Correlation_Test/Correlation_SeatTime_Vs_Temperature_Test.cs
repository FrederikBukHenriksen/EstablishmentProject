...
{
    public class Correlation_SeatTime_Vs_Temperature_Test : IntegrationTest
    {
        private IUnitOfWork unitOfWork;
        private IHandler<Correlation_SeatTime_Vs_Temperature_Command, CorrelationReturn> handler;
        private Establishment establishment;
        private Item testItem;


        public Correlation_SeatTime_Vs_Temperature_Test() : base(new List<ITestService> { DatabaseTestContainer.CreateAsync().Result })
        {
            //Inject services
            unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            handler = scope.ServiceProvider.GetRequiredService<IHandler<Correlation_SeatTime_Vs_Temperature_Command, CorrelationReturn>>();

            establishment = new Establishment("Cafe 1");
            testItem = establishment.CreateItem("test", 1);
            establishment.AddItem(testItem);
            establishment = Correlation_SeatTime_Vs_Temperature_Helper.CreateTestData(establishment, testItem);
        ...
        {
            //Arrange
            var command = new Correlation_SeatTime_Vs_Temperature_Command
            {
    ...
    public class Correlation_SeatTime_Vs_Temperature_Test_WithMockWeather : IntegrationTest
    {
        private IUnitOfWork unitOfWork;
        private IHandler<Correlation_SeatTime_Vs_Temperature_Command, CorrelationReturn> handler;
        private Establishment establishment;
        ...
        public Correlation_SeatTime_Vs_Temperature_Test_WithMockWeather() : base(new List<ITestService> { DatabaseTestContainer.CreateAsync().Result, new WeatherMock(Correlation_NumberOfSales_Vs_Temperature_Helper.testWeatherDataThatMatchSalesNumbers()) })
        {
            //Inject services
            unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            handler = scope.ServiceProvider.GetRequiredService<IHandler<Correlation_SeatTime_Vs_Temperature_Command, CorrelationReturn>>();
    ...
    public static class Correlation_SeatTime_Vs_Temperature_Helper
    {
        public static Establishment CreateTestData(Establishment establishment, Item item)
        {
            ...
            foreach (var distribution in aggregate.ToList())
            {
                for (int i = 0; i < distribution.Value; i++)
                {
                    ...
                    sale.setTimeOfArrival(distribution.Key.AddMinutes(distribution.Value * (-1)));
